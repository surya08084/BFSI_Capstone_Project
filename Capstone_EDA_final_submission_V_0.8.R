...
# Calclating missing % in demographic data    
...
# This is confirmed by TA 
#https://learn.upgrad.com/v/course/163/question/124008

demographic_data_V1<- demographic_data[-which(is.na(demographic_data$Performance.Tag)),]
nrow(demographic_data_V1)
# Now number of records in demographic_data_V1 is 69870 after removing NA values in performance tag

# Counting distinct values of application ID column
length(unique(demographic_data_V1$Application.ID))

# There are 69867 unique records in application id column which is same as number of record of demographic_data_V1 hence we can
# say Application.ID feature is here working as primary key (unique identifier)

#write.csv(demographic_data_V1,"demographic_data_V1.csv")
...
# 1. Age
quantile(demographic_data_V1$Age, seq(0, 1, 0.001))
...
nrow(filter(demographic_data_V1,demographic_data_V1$Age<0))

# As suggested by mentor if we have any wrong data like negative age values then that we will remove upfront and will not 
# treat those records.

demographic_data_V1<- demographic_data_V1[-which((demographic_data_V1$Age<0)),]

# Replacing Age between 0-18 with 18

demographic_data_V1$Age[which(demographic_data_V1$Age <= 18)] <- 18 

summary(demographic_data_V1$Age)

# Checking if there is any negative value of age is present

nrow(filter(demographic_data_V1,demographic_data_V1$Age<0))

quantile(demographic_data_V1$Age, seq(0, 1, 0.01))
...
levels(demographic_data_V1$Gender)
summary(demographic_data_V1$Gender)
# We have two levels F and M and 2 NA values

# 3.Marital.Status..at.the.time.of.application
levels(demographic_data_V1$Marital.Status..at.the.time.of.application)
summary(demographic_data_V1$Marital.Status..at.the.time.of.application)
# We have two levels Married and Single and 6 NA values

# 4.No.of.dependents

boxplot(demographic_data_V1$No.of.dependents)

nrow(filter(demographic_data_V1,demographic_data_V1$No.of.dependents<0))

summary(demographic_data_V1$No.of.dependents)

quantile(demographic_data_V1$No.of.dependents, seq(0, 1, 0.01),na.rm = T)
...
quantile(demographic_data_V1$Income, seq(0, 1, 0.01))

nrow(filter(demographic_data_V1,demographic_data_V1$Income<0))
...
demographic_data_V1<- demographic_data_V1[-which((demographic_data_V1$Income<0)),]

nrow(demographic_data_V1)
# Now there are 69785 reccords.
...
boxplot(demographic_data_V1$Income)
...
levels(demographic_data_V1$Education)
summary(demographic_data_V1$Education)

#There are 5 levels  "Bachelor","Masters","Others","Phd" and "Professional" and 118 NA values

summary(demographic_data_V1$Education)

# 7.Profession

summary(demographic_data_V1$Profession)
levels(demographic_data_V1$Profession)

#There are 3 levels : "SAL","SE" and "SE_PROF" and 13 NA values

#8.Type.of.residence
summary(demographic_data_V1$Type.of.residence)
levels(demographic_data_V1$Type.of.residence)
...
nrow(filter(demographic_data_V1,demographic_data_V1$No.of.months.in.current.residence<0))
summary(demographic_data_V1$No.of.months.in.current.residence)

# There is no negative value in the No.of.months.in.current.residence variable

quantile(demographic_data_V1$No.of.months.in.current.residence, seq(0, 1, 0.01))

boxplot(demographic_data_V1$No.of.months.in.current.residence)

#10. No.of.months.in.current.company
nrow(filter(demographic_data_V1,demographic_data_V1$No.of.months.in.current.company<0))
summary(demographic_data_V1$No.of.months.in.current.company)

# There is no negative value in the No.of.months.in.current.company variable

quantile(demographic_data_V1$No.of.months.in.current.company, seq(0, 1, 0.01))


#Now checking outlier with boxplot

boxplot(demographic_data_V1$No.of.months.in.current.company)

# There are outlier in the No.of.months.in.current.company variable which we will be treating using WOE transformation.

str(demographic_data_V1)

sum(is.na(demographic_data_V1))

# There are 150 NA values in the demographic_data_V1 dataset
...
#temp$Performance.Tag<-as.integer(temp$Performance.Tag)
...
## ----------------------  Model Deployment and Score card developemnt on the entire dataset ----------------------- 

pred <- predict(final_model, newdata = demographic_data_V1_final[, -c(2)], type = "response")
summary(pred)
...
pred_response <- factor(ifelse(pred >= 0.0415, "yes", "no"))

demographic_data_V1_final$Predicted_Performance.Tag <- pred_response
demographic_data_V1_final$Predicted_prob <- pred
View(demographic_data_V1_final)
demographic_data_V1_final$logit<- log(demographic_data_V1_final$Predicted_prob/(1-demographic_data_V1_final$Predicted_prob))
...
demographic_data_V1_final$Score = round(offset - factor * demographic_data_V1_final$logit)
...
#Let's remove the records which have NA values in target variable.
# This is confirmed by TA as well as mentor
#https://learn.upgrad.com/v/course/163/question/124008
...
credit_data_V1$Presence.of.open.auto.loan<-as.factor(credit_data_V1$Presence.of.open.auto.loan)

credit_data_V1$Presence.of.open.home.loan<-as.factor(credit_data_V1$Presence.of.open.home.loan)
...
quantile(credit_data_V1$No.of.times.90.DPD.or.worse.in.last.6.months, seq(0, 1, 0.01))

boxplot(credit_data_V1$No.of.times.90.DPD.or.worse.in.last.6.months)

summary(credit_data_V1$No.of.times.90.DPD.or.worse.in.last.6.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.times.90.DPD.or.worse.in.last.6.months<0))

# There are no unexpected values in this variable,outliers are there but that we will handel using WOE


#2.No.of.times.60.DPD.or.worse.in.last.6.months

quantile(credit_data_V1$No.of.times.60.DPD.or.worse.in.last.6.months, seq(0, 1, 0.01))

boxplot(credit_data_V1$No.of.times.60.DPD.or.worse.in.last.6.months)

summary(credit_data_V1$No.of.times.60.DPD.or.worse.in.last.6.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.times.60.DPD.or.worse.in.last.6.months<0))
...
quantile(credit_data_V1$No.of.times.30.DPD.or.worse.in.last.6.months, seq(0, 1, 0.01))

boxplot(credit_data_V1$No.of.times.30.DPD.or.worse.in.last.6.months)

summary(credit_data_V1$No.of.times.30.DPD.or.worse.in.last.6.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.times.30.DPD.or.worse.in.last.6.months<0))

# There are no unexpected values in this variable,outliers are there but that we will handel using WOE

# 4.No.of.times.90.DPD.or.worse.in.last.12.months

quantile(credit_data_V1$No.of.times.90.DPD.or.worse.in.last.12.months, seq(0, 1, 0.01))

boxplot(credit_data_V1$No.of.times.90.DPD.or.worse.in.last.12.months)

summary(credit_data_V1$No.of.times.90.DPD.or.worse.in.last.12.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.times.90.DPD.or.worse.in.last.12.months<0))

# There are no unexpected values in this variable,outliers are there but that we will handel using WOE

# 5.No.of.times.60.DPD.or.worse.in.last.12.months

quantile(credit_data_V1$No.of.times.60.DPD.or.worse.in.last.12.months, seq(0, 1, 0.01))

boxplot(credit_data_V1$No.of.times.60.DPD.or.worse.in.last.12.months)

summary(credit_data_V1$No.of.times.60.DPD.or.worse.in.last.12.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.times.60.DPD.or.worse.in.last.12.months<0))

# There are no unexpected values in this variable,outliers are there but that we will handel using WOE

# 6.No.of.times.30.DPD.or.worse.in.last.12.months
quantile(credit_data_V1$No.of.times.30.DPD.or.worse.in.last.12.months, seq(0, 1, 0.01))

boxplot(credit_data_V1$No.of.times.30.DPD.or.worse.in.last.12.months)

summary(credit_data_V1$No.of.times.30.DPD.or.worse.in.last.12.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.times.30.DPD.or.worse.in.last.12.months<0))
...
quantile(credit_data_V1$Avgas.CC.Utilization.in.last.12.months, seq(0, 1, 0.01),na.rm = T)

boxplot(credit_data_V1$Avgas.CC.Utilization.in.last.12.months)

summary(credit_data_V1$Avgas.CC.Utilization.in.last.12.months)

nrow(filter(credit_data_V1,credit_data_V1$Avgas.CC.Utilization.in.last.12.months<0))

# There are no unexpected values in this variable,outliers and NA vaues are there but that we will handel using WOE
# There are 1023 NA values.

# 8.No.of.trades.opened.in.last.6.months

quantile(credit_data_V1$No.of.trades.opened.in.last.6.months, seq(0, 1, 0.01),na.rm = T)

boxplot(credit_data_V1$No.of.trades.opened.in.last.6.months)

summary(credit_data_V1$No.of.trades.opened.in.last.6.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.trades.opened.in.last.6.months<0))

# There are no unexpected values in this variable,outliers and NA vaues are there but that we will handel using WOE
# There is 1 NA values.

# 9.No.of.trades.opened.in.last.12.months

quantile(credit_data_V1$No.of.trades.opened.in.last.12.months, seq(0, 1, 0.01),na.rm = T)

boxplot(credit_data_V1$No.of.trades.opened.in.last.12.months)

summary(credit_data_V1$No.of.trades.opened.in.last.12.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.trades.opened.in.last.12.months<0))

# There are no unexpected values in this variable,outliers are there but that we will handel using WOE

# 10.No.of.PL.trades.opened.in.last.6.months

quantile(credit_data_V1$No.of.PL.trades.opened.in.last.6.months, seq(0, 1, 0.01),na.rm = T)

boxplot(credit_data_V1$No.of.PL.trades.opened.in.last.6.months)

summary(credit_data_V1$No.of.PL.trades.opened.in.last.6.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.PL.trades.opened.in.last.6.months<0))
# There are no unexpected values in this variable,outliers are there but that we will handel using WOE

# 11.No.of.PL.trades.opened.in.last.12.months

quantile(credit_data_V1$No.of.PL.trades.opened.in.last.12.months, seq(0, 1, 0.01),na.rm = T)

boxplot(credit_data_V1$No.of.PL.trades.opened.in.last.12.months)

summary(credit_data_V1$No.of.PL.trades.opened.in.last.12.months)

nrow(filter(credit_data_V1,credit_data_V1$No.of.PL.trades.opened.in.last.12.months<0))
# There are no unexpected values in this variable,outliers are there but that we will handel using WOE


# 12.No.of.Inquiries.in.last.6.months..excluding.home...auto.loans.

quantile(credit_data_V1$No.of.Inquiries.in.last.6.months..excluding.home...auto.loans., seq(0, 1, 0.01),na.rm = T)

boxplot(credit_data_V1$No.of.Inquiries.in.last.6.months..excluding.home...auto.loans.)

summary(credit_data_V1$No.of.Inquiries.in.last.6.months..excluding.home...auto.loans.)

nrow(filter(credit_data_V1,credit_data_V1$No.of.Inquiries.in.last.6.months..excluding.home...auto.loans.<0))

# There are no unexpected values in this variable,outliers are there but that we will handel using WOE

# 13.No.of.Inquiries.in.last.12.months..excluding.home...auto.loans.

quantile(credit_data_V1$No.of.Inquiries.in.last.12.months..excluding.home...auto.loans., seq(0, 1, 0.01),na.rm = T)

boxplot(credit_data_V1$No.of.Inquiries.in.last.12.months..excluding.home...auto.loans.)

summary(credit_data_V1$No.of.Inquiries.in.last.12.months..excluding.home...auto.loans.)

nrow(filter(credit_data_V1,credit_data_V1$No.of.Inquiries.in.last.12.months..excluding.home...auto.loans.<0))
...
levels(credit_data_V1$Presence.of.open.home.loan)

summary(credit_data_V1$Presence.of.open.home.loan)
...
quantile(credit_data_V1$Outstanding.Balance, seq(0, 1, 0.01),na.rm = T)

boxplot(credit_data_V1$Outstanding.Balance)
summary(credit_data_V1$Outstanding.Balance)

nrow(filter(credit_data_V1,credit_data_V1$Outstanding.Balance<0))
...
quantile(credit_data_V1$Total.No.of.Trades, seq(0, 1, 0.01),na.rm = T)

boxplot(credit_data_V1$Total.No.of.Trades)
summary(credit_data_V1$Total.No.of.Trades)
nrow(filter(credit_data_V1,credit_data_V1$Total.No.of.Trades<0))
...
levels(credit_data_V1$Presence.of.open.auto.loan)

summary(credit_data_V1$Presence.of.open.auto.loan)

# There are no unexpected values in this variable and only two levels are present i.e. 0 and 1

# 18.Performance.Tag

summary(credit_data_V1$Performance.Tag)
...
############################# MERGING DEMOGRAPHIC & CREDIT BUREAU DATESETS #############################
#######################################################################################################
